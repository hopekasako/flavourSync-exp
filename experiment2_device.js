// Experiment 2: Taste Device Test Configuration
const experimentConfig = {
    experimentType: "taste_device",
    experimentName: "Experiment 2 - Taste Device Test",
    phases: {
        taste: {
            name: "Taste Phase",
            stimuli: [
                { id: 1, name: "Super Sweet", command: "TASTE_1" },
                { id: 2, name: "Medium Sweet", command: "TASTE_2" },
                { id: 3, name: "Little Sweet", command: "TASTE_3" },
                { id: 4, name: "Sour", command: "TASTE_4" },
                { id: 5, name: "Bitter", command: "TASTE_5" }
            ],
            questions: [
                {
                    id: "liking",
...
function updateConnectionStatus() {
    ...
    if (deviceConnected) {
        ...
        startButton.className = 'px-6 py-3 bg-primary text-white rounded-lg cursor-pointer hover:bg-green-700 transition-colors';
    } else {
...
function updateTestInstructions() {
    ...
    document.getElementById('test-instructions').textContent = `Please place the mouthpiece in your mouth. Click the button when ready to experience the taste.`;
}
...
function downloadResults() {
    ...
    const link = document.createElement('a');
    link.href = URL.createObjectURL(dataBlob);
    link.download = `experiment2_taste_device_participant${currentParticipant}_${new Date().toISOString().split('T')[0]}.json`;
    link.click();
}

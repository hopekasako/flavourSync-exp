...
function showScreen(screenId) {
  ...
  if (screenId === "test-screen") {
    ...
    if (activateBtn) {
      if (state.isManualTest) {
        activateBtn.textContent = "Move to Question Form";
        ...
        activateBtn.textContent = "Experience Stimulus";
        activateBtn.onclick = activateTest;
        activateBtn.disabled = false;
        activateBtn.style.display = "";
      }
    }
...
function updateTestInstructions() {
  ...
  // Thay đổi hướng dẫn nếu là manual test
  if (state.isManualTest) {
    instructions = `Manual mode: Please follow the manual instructions you received. When finished, proceed to the next form.`;
  } else {
    if (state.currentPhase === "smell") {
      instructions = `Please position your nose near the device. Click the button when ready to experience the smell.`;
    } else if (state.currentPhase === "taste") {
...
function showQuestion(questionNumber) {
  ...
  if (question.type === "text") {
    ...
    skipQuestionButton.className =
      "px-6 py-3 bg-gray-400 text-white rounded-lg shadow hover:bg-gray-500 transition-colors text-lg font-semibold";
    ...
    skipTrialButton.className =
      "px-6 py-3 bg-gray-500 text-white rounded-lg shadow hover:bg-gray-600 transition-colors text-lg font-semibold";
    ...
    skipPhaseButton.className =
      "px-6 py-3 bg-red-500 text-white rounded-lg shadow hover:bg-red-600 transition-colors text-lg font-semibold";
  ...
  if (question.id !== "liking") {
    ...
    const intensityLabels = [
      ...
      { text: "Strong", value: 33.113, pos: 0.4 },
      { text: "Moderate", value: 16.218, pos: 0.6 },
      { text: "Weak", value: 5.754, pos: 0.8 },
      { text: "Barely detectable", value: 1.38, pos: 1 },
    ];
...
function skipQuestion() {
  ...
  const phaseQuestions =
    experimentConfig.phases[state.currentPhase].questions.length;
...
function downloadResults() {
  ...
  const output = {
    ...
    mode: state.isManualTest ? "manual" : "system",
  };
  ...
  let filename;
  if (state.isManualTest) {
    filename = `manual__results_participant_${state.participantNumber}.json`;
  } else {
    filename = `system_results_participant_${state.participantNumber}.json`;
  }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  addEventListenerIfExists("manual-next-btn", "click", function () {
    showScreen("survey-screen");
    createSurveyQuestions();
  });

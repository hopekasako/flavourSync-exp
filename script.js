...
const state = {
    ...
    currentFlavorPart: 'orthonasal', // 'orthonasal' or 'retronasal'
    currentFlavorTrial: 1, // 1 or 2
    testMode: null // 'manual' or 'system'
};
...
function showScreen(screenId) {
    ...
    document.getElementById('welcome-screen').classList.add('hidden');
    document.getElementById('test-screen').classList.add('hidden');
    document.getElementById('manual-test-screen').classList.add('hidden');
    document.getElementById('test-mode-screen').classList.add('hidden');
    document.getElementById('survey-screen').classList.add('hidden');
    document.getElementById('break-screen').classList.add('hidden');
    document.getElementById('completed-screen').classList.add('hidden');
    document.getElementById(screenId).classList.remove('hidden');
...
function updateExperimentStatus() {
    ...
    document.getElementById('stimulus-indicator').textContent = `${state.currentStimulus} of ${totalStimuli}`;
    // Manual test screen
    document.getElementById('manual-section-title').textContent = `${phase.name} - Manual Condition`;
    document.getElementById('manual-phase-indicator').textContent = phase.name;
    document.getElementById('manual-trial-indicator').textContent = `${state.currentTrial} of 3`;
    document.getElementById('manual-stimulus-indicator').textContent = `${state.currentStimulus} of ${phase.stimuli.length}`;
    document.getElementById('participant-status').textContent = `Participant: ${state.participantNumber}`;
}
...
function updateManualTestScreen() {
    const phase = experimentConfig.phases[state.currentPhase];
    document.getElementById('manual-section-title').textContent = `${phase.name} - Manual Condition`;
    document.getElementById('manual-phase-indicator').textContent = phase.name;
    document.getElementById('manual-trial-indicator').textContent = `${state.currentTrial} of 3`;
    ...
    let instructions = '';
    if (state.currentPhase === 'smell') {
        instructions = `Please position your nose near the device. When ready, click "Move to Survey Form".`;
    } else if (state.currentPhase === 'taste') {
        instructions = `Please position your mouth in the mouthpiece. When ready, click "Move to Survey Form".`;
    } else if (state.currentPhase === 'flavor') {
        instructions = `Please position your nose near the device and mouth in the mouthpiece. When ready, click "Move to Survey Form".`;
    }
    document.getElementById('manual-test-instructions').textContent = instructions;
}

// Test mode selection
function showTestModeScreen() {
    showScreen('test-mode-screen');
}
function showManualTestScreen() {
    state.testMode = 'manual';
    showScreen('manual-test-screen');
    updateManualTestScreen();
}
function showSystemTestScreen() {
    state.testMode = 'system';
    showScreen('test-screen');
    updateTestInstructions();
    updateExperimentStatus();
}
function moveToSurveyFromManual() {
    state.currentQuestion = 1;
    showScreen('survey-screen');
    createSurveyQuestions();
}

// Survey and question logic (unchanged, see your previous code for full details)
function createSurveyQuestions() {
    const phase = experimentConfig.phases[state.currentPhase];
    if (!phase || !phase.questions || phase.questions.length === 0) {
        console.error('No questions found for phase:', state.currentPhase);
...
async function activateTest() {
    ...
    if (Array.isArray(commands)) {
        for (const cmd of commands) {
            if (cmd.delay > 0) await new Promise(resolve => setTimeout(resolve, cmd.delay));
            await sendCommand(cmd.command);
        }
    } else if (commands) {
        await sendCommand(commands);
...
// Move to next step (after survey)
function moveToNextStep() {
    const phase = experimentConfig.phases[state.currentPhase];
    if (state.currentPhase === 'flavor') {
        const totalStimuli = phase.stimuli.length;
...
// Skip trial/phase logic (unchanged)
function skipTrial() {
    if (confirm('Are you sure you want to skip this trial? All answers will be saved.')) {
        if (state.currentPhase === 'flavor') {
            const totalStimuli = experimentConfig.phases.flavor.stimuli.length;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const savedData = localStorage.getItem('flavourSyncAnswers');
    if (savedData) {
        try { state.answers = JSON.parse(savedData); } catch (e) { }
    }
    function addEventListenerIfExists(elementId, event, handler) {
        const element = document.getElementById(elementId);
    ...
    addEventListenerIfExists('start-participant-btn', 'click', startParticipant);
    addEventListenerIfExists('connect-device-btn', 'click', connectToDevice);
    addEventListenerIfExists('start-experiment-btn', 'click', showTestModeScreen);
    addEventListenerIfExists('manual-test-btn', 'click', showManualTestScreen);
    addEventListenerIfExists('system-test-btn', 'click', showSystemTestScreen);
    addEventListenerIfExists('move-to-survey-btn', 'click', moveToSurveyFromManual);
    addEventListenerIfExists('activate-btn', 'click', activateTest);
    addEventListenerIfExists('submit-survey-btn', 'click', handleSubmitSurvey);
    addEventListenerIfExists('skip-survey-btn', 'click', skipSurvey);
    addEventListenerIfExists('continue-btn', 'click', continueAfterBreak);
